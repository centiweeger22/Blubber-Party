...
public class ServerLinker
{
	...
	public ServerLinker(ReadyFunc _readyFunction, IPAddress _remoteIp, int _remotePort, int _bindingPort)
	{
		channel = new Channel();
	...
	public void ProcessNetworkMessages()
	{
		lock (messageBuffer)
		{
			foreach (RecievedMessage item in messageBuffer)
			{
				...
				if (state == ELinkerState.WAITING && eLinkingStateType == Settings.ELinkingStateType.LOCAL_IP_DETECTED)
				{
					forcePublicIp = true;
				...
				else if (state == ELinkerState.WAITING && eLinkingStateType == Settings.ELinkingStateType.LINK)
				{
